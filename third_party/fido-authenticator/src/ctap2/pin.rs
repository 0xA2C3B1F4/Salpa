...
// CTAP 2.3 sections 6.5.2.1 and 6.5.3.2. RissoKey uses USB, without a
// rolling timer or cached user presence. Successful authorization within the
// first 30 seconds permits use until the absolute 600-second limit.
const INITIAL_USAGE_LIMIT: Duration = Duration::from_secs(30);
const MAX_USAGE_PERIOD: Duration = Duration::from_secs(600);
...
impl PinToken {
    ...
    pub fn require_permissions(&self, permissions: Permissions) -> Result<()> {
        if self.state.is_in_use && self.state.permissions.contains(permissions) {
            Ok(())
        } else {
    ...
    pub fn require_valid_for_rp(&self, scope: RpScope<'_>) -> Result<()> {
        if self.state.is_in_use && self.is_valid_for_rp(scope) {
            Ok(())
        } else {
            Err(Error::PinAuthInvalid)
        }
    ...
    /// Call only after the operation's permission and RP checks pass. A valid
    /// HMAC for a disallowed operation must not extend the initial lifetime.
    pub fn mark_used(&mut self) {
        self.state.has_been_used = true;
    }

    pub fn require_user_verified(&self) -> Result<()> {
        if self.state.is_in_use && self.state.is_user_verified {
            Ok(())
        } else {
            Err(Error::PinAuthInvalid)
        }
    ...
    pub fn bind_rp_if_unset(&mut self, rp_id: &str) -> Result<()> {
        if self.state.rp.is_none() {
            use sha2::{Digest, Sha256};
            self.state.rp = Some(Rp {
                id: String::try_from(rp_id).map_err(|_| Error::InvalidParameter)?,
                hash: Sha256::digest(rp_id.as_bytes()).into(),
            });
        }
        self.require_valid_for_rp(RpScope::RpId(rp_id))
    }
}
...
impl PinTokenState {
    fn observe(&mut self, now: Duration) {
        let elapsed = self.issued_at.and_then(|issued| now.checked_sub(issued));
        if !self.is_in_use
            || now < self.last_observed
            || elapsed.is_none()
            || elapsed.is_some_and(|elapsed| {
                elapsed >= MAX_USAGE_PERIOD
                    || (!self.has_been_used && elapsed >= INITIAL_USAGE_LIMIT)
            })
        {
            // stopUsingPinUvAuthToken resets all state, including RP scope,
            // permissions, timer and UV/UP flags. The caller also deletes its key.
            *self = Self::default();
        } else {
            self.last_observed = now;
            self.is_user_present = false; // user-present time limit is zero
        }
    }
}
...
impl PinProtocolState {
    ...
    pub fn observe_usage_timer<T: CryptoClient>(&mut self, trussed: &mut T, now: Duration) -> bool {
        let mut expired = false;
        for slot in [&mut self.pin_token_v1, &mut self.pin_token_v2] {
            if let Some(token) = slot.as_mut() {
                token.state.observe(now);
                if !token.state.is_in_use {
                    slot.take().unwrap().delete(trussed);
                    expired = true;
                }
            }
        }
    ...
    pub fn reset_pin_tokens<T: CryptoClient>(&mut self, trussed: &mut T) {
        for slot in [&mut self.pin_token_v1, &mut self.pin_token_v2] {
    ...
    pub fn consume_user_presence(&mut self) {
        for slot in [&mut self.pin_token_v1, &mut self.pin_token_v2] {
            if let Some(token) = slot.as_mut() {
                token.state.is_user_present = false;
                token.state.is_user_verified = false;
                token.state.permissions &= Permissions::LARGE_BLOB_WRITE;
            }
        }
    }
}
...
impl<'a, T: CryptoClient + ManagementClient + HkdfClient + HmacSha256 + P256> PinProtocol<'a, T> {
    pub fn new(
        trussed: &'a mut T,
        state: &'a mut PinProtocolState,
        version: PinProtocolVersion,
    ...
    fn pin_token_mut(&mut self) -> Option<&mut PinToken> {
        match self.version {
            PinProtocolVersion::V1 => self.state.pin_token_v1.as_mut(),
            PinProtocolVersion::V2 => self.state.pin_token_v2.as_mut(),
        }
    }

    fn observe_usage_timer(&mut self) -> Result<()> {
        match try_syscall!(self.trussed.uptime()) {
            Ok(reply) => {
                self.state.observe_usage_timer(self.trussed, reply.uptime);
            }
            Err(_) => {
    ...
    pub fn reset_pin_tokens(&mut self) {
        self.state.reset_pin_tokens(self.trussed);
    }

    // in spec: beginUsingPinUvAuthToken(userIsPresent)
    fn begin_using_pin_token(&mut self, _is_user_present: bool) -> Result<PinTokenMut<'_, T>> {
        ...
        pin_token.state.is_user_present = false;
        pin_token.state.is_user_verified = true;
        pin_token.state.issued_at = Some(now);
        pin_token.state.last_observed = now;
    ...
    pub fn reset_and_begin_using_pin_token(
        &mut self,
        is_user_present: bool,
    ) -> Result<PinTokenMut<'_, T>> {
        self.reset_pin_tokens();
    ...
    fn verify(&mut self, key: KeyId, data: &[u8], signature: &[u8]) -> bool {
        ...
        bool::from(expected_signature.ct_eq(signature))
    }

    // in spec: verify(pinUvAuthToken, ...)
    pub fn verify_pin_token(&mut self, data: &[u8], signature: &[u8]) -> Result<&mut PinToken> {
        self.observe_usage_timer()?;
