...
struct ResidentReplacement {
    old_path: PathBuf,
    old_key: KeyId,
    new_path: PathBuf,
}
const RK_STAGE: &Path = path!("rk-new");
const RK_JOURNAL_STAGE: &Path = path!("rk-transaction-new");
const RK_JOURNAL: &Path = path!("rk-transaction");
...
impl<UP: UserPresence, T: TrussedRequirements> Authenticator for crate::Authenticator<UP, T> {
    ...
    fn get_info(&mut self) -> ctap2::get_info::Response {
        ...
        if self.config.ctap1_transport && !self.state.persistent.always_uv() {
        ...
        // Not `::ALL`: ctap-types enables mldsa44 for PIV too, we only sign what's here.
        let mut algorithms = Vec::new();
        algorithms
            .push(KnownPublicKeyCredentialParameters::ES256)
            .unwrap();
        algorithms
            .push(KnownPublicKeyCredentialParameters::EdDSA)
            .unwrap();
        #[cfg(feature = "mldsa44")]
        algorithms
            .push(KnownPublicKeyCredentialParameters::MLDSA44)
            .unwrap();
        ...
        response.aaguid = ByteArray::new(aaguid);
        ...
        cfg_cmds
            .push(ctap2::config::Subcommand::ToggleAlwaysUv)
            .unwrap();
        cfg_cmds
            .push(ctap2::config::Subcommand::SetMinPINLength)
            .unwrap();
        if self.config.long_touch_for_reset {
            cfg_cmds
                .push(ctap2::config::Subcommand::EnableLongTouchForReset)
                .unwrap();
        }
    ...
    #[inline(never)]
    fn get_next_assertion_into(
        &mut self,
        response: &mut ctap2::get_assertion::Response,
    ) -> Result<()> {
        let now = try_syscall!(self.trussed.uptime())
            .ok()
            .map(|reply| reply.uptime);
        let valid = self
            .state
            .runtime
            .active_get_assertion
            .as_ref()
            .and_then(|active| now.and_then(|now| now.checked_sub(active.last_assertion)))
            .is_some_and(|elapsed| elapsed <= core::time::Duration::from_secs(30));
        if !valid {
        ...
        self.assert_with_credential(None, &Credential::Full(credential), response)?;
        self.refresh_assertion_timer()
    }
    ...
    fn make_credential_into(
        ...
        // CTAP §6.1.2: first supported algorithm wins, hence the `is_none` guards.
        ...
        self.up
            .user_present(&mut self.trussed, self.config.fido2_up_timeout())?;
        self.state.runtime.consume_token_user_presence();
        ...
        let make_credential_result = (|| -> Result<()> {
            ...
            let att_stmt = if let Some(format) = att_stmt_fmt {
                match format {
                    ...
                    SupportedAttestationFormat::Packed => {
                        // Sign auth_data ‖ cdh in place, then truncate back to auth_data.
            ...
            // Commit only after the complete attestation response is ready.
            if rk_requested {
                // serialization with all metadata
                let serialized_credential = credential.serialize()?;

                let old = self.resident_key_by_user_id(&rp_id_hash, credential.user.id())?;
                let count = CredentialManagement::new(self).count_credentials()?;
                if self.can_fit(serialized_credential.len()) == Some(false)
                    || (old.is_none()
                        && count
                            >= self
                                .config
                ...
                let new_path = rk_path(&rp_id_hash, &credential_id_hash);
                if let Some((old_path, old_key)) = old {
                    // The journal rename is the commit point. Until then the old
                    // credential is untouched. After it, recovery owns the new key.
                    ...
                    let journal = ResidentReplacement {
                        old_path,
                        old_key,
                        new_path,
                    };
                    let mut bytes = Message::new();
                    cbor_smol::cbor_serialize_to(&journal, &mut bytes).map_err(|_| Error::Other)?;
                    try_syscall!(self.trussed.write_file(
                        Location::Internal,
                        RK_JOURNAL_STAGE.into(),
                        bytes,
                        None
                    ))
                    .map_err(|_| Error::KeyStoreFull)?;
                    // A failed syscall may still have committed its storage operation.
                    // Do not delete the new key after attempting the commit point.
                    replacement_committed = true;
                    try_syscall!(self.trussed.rename(
                        Location::Internal,
                        RK_JOURNAL_STAGE.into(),
                        RK_JOURNAL.into()
                    ))
                    .map_err(|_| Error::Other)?;
                    self.recover_resident_replacement()?;
                } else {
                    try_syscall!(self.trussed.write_file(
                        Location::Internal,
                        new_path,
        ...
        // Once the replacement commit was attempted, recovery owns the new key
        // even if the storage syscall reported an error after committing it.
        // Earlier failures can safely discard the new key.
        if make_credential_result.is_err() && !replacement_committed {
    ...
    fn reset(&mut self) -> Result<()> {
        ...
        // Invalidate volatile authorizations before any destructive state write.
        self.state.runtime.invalidate_pin_tokens(&mut self.trussed);
    ...
    fn client_pin(
        ...
        match parameters.sub_command {
            ...
            Subcommand::ChangePin => {
                ...
                // front and compare with subtle's constant-time operation.
                // Only return the error when force_pin_change is
                ...
                if self.state.persistent.force_pin_change()
                    && self
                        ...
                        .persistent
                        .pin_hash()
                        .is_some_and(|stored| bool::from(stored.ct_eq(&new_pin_hash)))
                {
                ...
                // Invalidate before persistent changes: a later storage error
                // must not leave the previous PIN's token usable.
                self.state.runtime.invalidate_pin_tokens(&mut self.trussed);
            ...
            Subcommand::GetPinToken => {
                ...
                // 12. Reset all PIN tokens and their continuation state.
                self.state.runtime.invalidate_pin_tokens(&mut self.trussed);
            ...
            Subcommand::GetPinUvAuthTokenUsingPinWithPermissions => {
                ...
                // CTAP 2.3 pcmr is defined, but this implementation does not
                // advertise or implement persistent read-only management tokens.
    ...
    fn get_assertion_into(
        ...
        self.state.runtime.active_get_assertion = Some(state::ActiveGetAssertionData {
            last_assertion: core::time::Duration::ZERO,
            rp_id_hash: {
                let mut buf = [0u8; 32];
...
impl<UP: UserPresence, T: TrussedRequirements> crate::Authenticator<UP, T> {
    ...
    fn decrypt_pin_hash_and_maybe_escalate(
        ...
        let pin_hash = shared_secret.decrypt(&mut self.trussed, pin_hash_enc);
        ...
        if !pin_hash
            .as_ref()
            .is_some_and(|hash| bool::from(hash.as_slice().ct_eq(&stored_pin_hash)))
        {
            // I) generate new KEK
        ...
        match parameters.sub_command {
            ...
            | Subcommand::UpdateUserInformation => {
                ...
                if let Ok(pin_token) = pin_protocol.verify_pin_token(&data[..len], pin_auth) {
                    ...
                    // CTAP 2.3 section 6.8: a rejected token authenticator is
                    // not an incorrect PIN entry and consumes no PIN retries.
        ...
        if self.state.persistent.pin_is_set() {
            ...
            if let Some(pin_auth) = pin_auth {
                ...
                if let Some(pin_protocol) = pin_protocol {
                    ...
                    pin_token.require_user_verified()?;
                    pin_token.bind_rp_if_unset(rp_id)?;
                    pin_token.mark_used();
    ...
    fn assert_with_credential(
        ...
        let assertion_result = (|| -> Result<()> {
            ...
            // Sign auth_data ‖ cdh in place, truncated back after the att_stmt.
            let auth_data_len = serialized_auth_data.len();
            serialized_auth_data
    ...
    /// Write a `Packed` attestation statement into `response.att_stmt`.
    /// Outlined to keep its ~4.5 KB of temporaries out of the caller's frame.
    #[inline(never)]
    ...
    #[inline(never)]
    pub(crate) fn recover_resident_replacement(&mut self) -> Result<()> {
        let present = try_syscall!(self
            .trussed
            .entry_metadata(Location::Internal, RK_JOURNAL.into()))
        .map_err(|_| Error::Other)?
        .metadata
        .is_some();
        if !present {
        ...
        let bytes = try_syscall!(self
            .trussed
            .read_file(Location::Internal, RK_JOURNAL.into()))
        .map_err(|_| Error::Other)?
        .data;
        let journal: ResidentReplacement =
            cbor_smol::cbor_deserialize(&bytes).map_err(|_| Error::Other)?;
        if journal.old_path == journal.new_path
            || journal.old_path.parent().as_deref() != Some(RK_DIR)
            || journal.new_path.parent().as_deref() != Some(RK_DIR)
        {
            return Err(Error::Other);
        }
        let staged = try_syscall!(self
            .trussed
            .entry_metadata(Location::Internal, RK_STAGE.into()))
        .map_err(|_| Error::Other)?
        .metadata
        .is_some();
        if staged {
            try_syscall!(self.trussed.rename(
                Location::Internal,
                RK_STAGE.into(),
                journal.new_path.clone()
            ))
            .map_err(|_| Error::Other)?;
        }
        // Never remove the old credential unless the published record is readable.
        let new = try_syscall!(self.trussed.read_file(Location::Internal, journal.new_path))
            .map_err(|_| Error::Other)?
            .data;
        let credential = FullCredential::deserialize(&new).map_err(|_| Error::Other)?;
        if !matches!(credential.key, Key::ResidentKey(key) if key != journal.old_key) {
            return Err(Error::Other);
        }
        if try_syscall!(self
            .trussed
            .entry_metadata(Location::Internal, journal.old_path.clone()))
        .map_err(|_| Error::Other)?
        .metadata
        .is_some()
        {
            try_syscall!(self
                .trussed
                .remove_file(Location::Internal, journal.old_path))
            .map_err(|_| Error::Other)?;
        }
        // Delete is idempotent: a missing key reports success=false, not an error.
        try_syscall!(self.trussed.delete(journal.old_key)).map_err(|_| Error::Other)?;
        try_syscall!(self
            .trussed
            .remove_file(Location::Internal, RK_JOURNAL.into()))
        .map_err(|_| Error::Other)?;
        Ok(())
    }

    fn refresh_assertion_timer(&mut self) -> Result<()> {
        match try_syscall!(self.trussed.uptime()) {
            Ok(reply) => {
                if let Some(active) = self.state.runtime.active_get_assertion.as_mut() {
                    active.last_assertion = reply.uptime;
                }
                Ok(())
            }
            Err(_) => {
    ...
    fn resident_key_by_user_id(
        &mut self,
        rp_id_hash: &[u8; 32],
        user_id: &Bytes<64>,
    ) -> Result<Option<(PathBuf, KeyId)>> {
        ...
        while let Some(entry) = maybe_entry.take() {
            ...
            if let Ok(old_credential) = credential_maybe {
                if old_credential.user.id() == user_id {
                    return match old_credential.key {
                        credential::Key::ResidentKey(key) => Ok(Some((rk_path, key))),
                        _ => Err(Error::Other),
                    };
                }
            } else {
