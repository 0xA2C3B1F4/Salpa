...
with tempfile.TemporaryDirectory(prefix="salpa-key-helper-test-") as tmp:
    folder = Path(tmp)
    for address in (0, 16, 112, 0x1000, 0x20000, 0x200000):
        output = io.BytesIO()
        espsecure._flash_encryption_operation_aes_xts(
            output,
            io.BytesIO(bytes(range(256))),
            address,
            io.BytesIO(bytes(range(32))),
            False,
        )
        (folder / f"xts-{address}.bin").write_bytes(output.getvalue())
    (folder / "pbkdf2.bin").write_bytes(
        hashlib.pbkdf2_hmac(
            "sha256", b"test-only-public-passphrase", bytes([42]) * 32, 600000, 32
        )
    )
    ...
    for scalar in (1, 2, 3, 4, 5):
        ...
        subject = x509.Name(
            [x509.NameAttribute(NameOID.COMMON_NAME, "PUBLIC TEST FIXTURE ONLY")]
        )
        ...
        certificate_builder = (
            ...
            .not_valid_after(today + timedelta(days=1))
        )
        if scalar != 4:
            aaguid = (
                bytes(16)
                if scalar == 3
                else bytes.fromhex("989e2cc205df4e64806822683886e8b3")
            )
            certificate_builder = certificate_builder.add_extension(
                x509.UnrecognizedExtension(
                    ObjectIdentifier("1.3.6.1.4.1.45724.1.1.4"), b"\x04\x10" + aaguid
                ),
                critical=scalar == 5,
            )
        ...
        if scalar == 1:
            ...
            espsecure._flash_encryption_operation_aes_xts(
                output,
                io.BytesIO(attestation_payload),
                0x200000,
                io.BytesIO(bytes(range(32))),
                False,
            )
    ...
    (folder / "main.swift").write_text(
        (root / "main.swift").read_text() + (root / "test.swift.inc").read_text()
    )
    subprocess.run(
        [
            "swiftc",
            ...
            "KEY_HELPER_TEST",
            "-module-cache-path",
            str(folder / "module-cache"),
            str(folder / "main.swift"),
            "-o",
            str(folder / "test-helper"),
        ],
        check=True,
    )
    subprocess.run([str(folder / "test-helper"), str(folder)], check=True)
    subprocess.run(
        [str(folder / "test-helper"), str(folder), "restore-attestation-fixture"],
        check=True,
    )
    ...
    derived = hashlib.pbkdf2_hmac(
        "sha256", b"test-only-public-passphrase", salt, 600000, 32
    )
    aad = "|".join(
        envelope[field]
        for field in ("format", "role", "certificateSHA256", "publicKeyX963SHA256")
    ).encode()
    sealed = base64.b64decode(envelope["sealed"])
    assert AESGCM(derived).decrypt(sealed[:12], sealed[12:], aad) == attestation_payload
    print(
        "PASS: separate-process attestation restore; "
        "Python authenticated backup decryption; exact staging contract"
    )

    key.public_key().verify(
        (folder / "test.sig").read_bytes(),
        bytes(range(256)),
        padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=32),
        hashes.SHA256(),
    )
    print(
        "PASS: Swift RSA-PSS verified by cryptography with Espressif salt length 32"
    )

    spec = importlib.util.spec_from_file_location(
        "attestation_generator", root.parent / "generate-dev-attestation.py"
    )
    ...
    with (
        mock.patch(
            "sys.argv", ["generate-dev-attestation.py", "--quiet", str(generated)]
        ),
        mock.patch.object(
            generator.ec,
            "generate_private_key",
            return_value=ec.derive_private_key(1, ec.SECP256R1()),
        ),
    ...
    assert manifest["certificate_sha256"] == hashlib.sha256(
        (generated / "attestation-cert.der").read_bytes()
    ).hexdigest()
    print(
        "PASS: quiet generator and disabled core dumps, "
        "using only public scalar-one fixture"
    )
