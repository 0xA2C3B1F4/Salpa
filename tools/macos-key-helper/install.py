...
parser.add_argument(
    '--identity',
    required=True,
    help='Existing macOS code-signing certificate SHA-1; not a firmware key',
)
...
with tempfile.TemporaryDirectory(prefix='salpa-key-helper-build-') as temp:
    folder = Path(temp)
    ...
    subprocess.run(
        [
            ...
            'runtime',
            '--identifier',
            'fi.rissotek.rissokey.keyhelper',
            str(binary),
        ],
        check=True,
    )
