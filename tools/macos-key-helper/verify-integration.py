...
subprocess.run([sys.executable, '-m', 'espsecure', 'verify-signature', '--version', '2', '--keyfile', str(args.public_key), str(args.existing_signed_image)], check=True, capture_output=True)
public = serialization.load_pem_public_key(args.public_key.read_bytes())
fingerprint = hashlib.sha256(public.public_bytes(serialization.Encoding.DER, serialization.PublicFormat.PKCS1)).hexdigest()
inventory = Path.home() / 'Library/Application Support/RissoKey/key-inventory.json'
if json.loads(inventory.read_text())['keys']['signing']['public_key_pkcs1_sha256'] != fingerprint:
...
KeychainSigningKey(args.helper, args.public_key).sign_image(Path(sys.executable), unsigned, signed)
subprocess.run([sys.executable, '-m', 'espsecure', 'verify-signature', '--version', '2', '--keyfile', str(args.public_key), str(signed)], check=True, capture_output=True)
...
if cipher.read_bytes() != args.existing_ciphertext.read_bytes():
    raise SystemExit('Ciphertext differs; check that the plaintext is the exact packaged artifact')
result = {'schema': 1, 'verified_at': datetime.now(timezone.utc).isoformat(), 'existing_public_root_matched': True, 'keychain_rsa_signature_verified_by_espsecure': True, 'flash_ciphertext_matched_current_package': True, 'hardware_operations_performed': False}
(args.output_dir / 'result.json').write_text(json.dumps(result, indent=2) + '\n')
print('PASS: Keychain signature matches the existing trust root; ciphertext matches the existing package')
