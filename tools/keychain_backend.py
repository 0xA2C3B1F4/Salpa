...
class KeychainSigningKey:
    ...
    def sign_image(self, python: Path, source: Path, output: Path) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="rissokey-sign-") as folder:
            ...
            subprocess.run([str(python), "-m", "espsecure", "sign-data", "--version", "2", "--pub-key", str(self.public_key), "--signature", str(signature), "--output", str(output), str(padded)], check=True)
...
class KeychainFlashKey:
    ...
    def encrypt(self, plaintext: Path, ciphertext: Path, address: int) -> None:
        subprocess.run([str(self.helper), "encrypt", hex(address), str(plaintext), str(ciphertext)], check=True)
