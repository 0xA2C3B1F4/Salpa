...
def create_resident(vid: int, pid: int, state_path: Path) -> None:
    ...
    try:
        ...
        response = ctap.make_credential(
            client_data_hash,
            {"id": RP_ID, "name": "RissoKey resident probe"},
            {
                "id": state[K_USER_ID],
                "name": "resident-probe",
                "displayName": "RissoKey resident probe",
            },
...
def attest_after_reset(vid: int, pid: int, state_path: Path) -> None:
    ...
    try:
        ...
        response = ctap.make_credential(
            client_data_hash,
            {"id": "reset.rissokey.invalid", "name": "RissoKey reset probe"},
            {
                "id": os.urandom(32),
                "name": "reset-probe",
                "displayName": "RissoKey reset probe",
            },
            [{"type": "public-key", "alg": -7}],
            options={"rk": False},
            on_keepalive=user_presence_notice(),
