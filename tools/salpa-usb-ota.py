#!/usr/bin/env python3
"""Install a signed ESP32-S2 Salpa image through CTAPHID vendor command 0x51."""
...
def require_ok(status: DeviceStatus, operation: str) -> None:
    if status.status == 13:
        raise UpdateError(
            f"{operation} failed: session-expired; restart the update and approve "
            "it with a fresh button press"
        )
    if status.status != 0:
...
def install(device: Any, image: bytes, metadata: ImageMetadata) -> None:
    ...
    def keepalive(_status: object) -> None:
        nonlocal prompted
        if not prompted:
            print("Press and release the Salpa update button to approve this image.", flush=True)
            prompted = True
