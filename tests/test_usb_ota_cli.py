...
SPEC = importlib.util.spec_from_file_location(
    "salpa_usb_ota", ROOT / "tools" / "salpa-usb-ota.py"
)
...
class UpdaterEntrypointTests(unittest.TestCase):
    def test_command_help_and_argument_validation(self):
        for arguments, expected_code in ((["--help"], 0), (["--not-an-option"], 2)):
            with self.subTest(arguments=arguments):
                result = subprocess.run(
                    [sys.executable, str(ROOT / "tools" / "salpa-usb-ota.py"), *arguments],
                    capture_output=True,
                    text=True,
                    check=False,
                )
                ...
                self.assertIn("salpa-usb-ota.py", output)
...
class UsbOtaCliTests(unittest.TestCase):
    def test_expired_session_requires_restart_without_automatic_reapproval(self) -> None:
        image = image_bytes()
        metadata = OTA.ImageMetadata(len(image), hashlib.sha256(image).digest(), 3, "2.4.0")

        class ExpiredDevice(FakeDevice):
            def call(self, command: int, request: bytes, on_keepalive=None) -> bytes:
                if request[0] == OTA.OP_WRITE:
                    self.requests.append(request)
                    return reply(phase=5, session=7, status=13, slot=1)
                return super().call(command, request, on_keepalive)

        device = ExpiredDevice(len(image))
        with redirect_stdout(io.StringIO()):
            with self.assertRaisesRegex(OTA.UpdateError, "session-expired.*fresh button press"):
                OTA.install(device, image, metadata)
        self.assertEqual(sum(request[0] == OTA.OP_BEGIN for request in device.requests), 1)
        self.assertEqual(device.requests[-1][0], OTA.OP_WRITE)
