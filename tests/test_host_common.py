...
class EnvironmentAliasTests(unittest.TestCase):
    def test_current_and_legacy_inputs_normalize_to_the_same_environment(self):
        current = {"PATH": "toolchain", "SALPA_USB_SERIAL": "fixture-only"}
        legacy = {"PATH": "toolchain", "RISSO_KEY_USB_SERIAL": "fixture-only"}
        self.assertEqual(salpa_environment(current), current)
        self.assertEqual(salpa_environment(legacy), current)
        self.assertEqual(salpa_environment(current | legacy), current)
        self.assertIn("RISSO_KEY_USB_SERIAL", legacy)

    def test_conflicts_fail_without_including_input_values(self):
        values = {"SALPA_DEV_ATTESTATION_KEY": "first-private-path",
                  "RISSO_KEY_DEV_ATTESTATION_KEY": "second-private-path"}
        with self.assertRaises(ValueError) as raised:
            salpa_environment(values)
        message = str(raised.exception)
        self.assertIn("SALPA_DEV_ATTESTATION_KEY", message)
        for value in values.values():
            self.assertNotIn(value, message)

    def test_empty_new_input_does_not_silently_restore_old_acknowledgement(self):
        with self.assertRaises(ValueError):
            salpa_environment({"SALPA_PROVISIONING_ACK": "",
                               "RISSO_KEY_PROVISIONING_ACK": "ERASE_FIDO_STORE"})

    def test_identity_mapping_keeps_unrelated_fields_and_numeric_values(self):
        self.assertEqual(salpa_environment({"RISSO_KEY_USB_PID": 1, "schema": 1}),
                         {"SALPA_USB_PID": 1, "schema": 1})
