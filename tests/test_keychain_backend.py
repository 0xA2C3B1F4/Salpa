...
spec = importlib.util.spec_from_file_location('keychain_test_backend', ROOT / 'tools/keychain_backend.py')
...
class KeychainBackendTests(unittest.TestCase):
    ...
    def test_cancel_stops_before_packaging(self):
        with tempfile.TemporaryDirectory() as folder:
            source = Path(folder) / 'image.bin'; source.write_bytes(b'image')
            with patch.object(BACKEND.subprocess, 'run', side_effect=subprocess.CalledProcessError(1, ['helper'])) as run:
    ...
    def test_padding_and_external_signature_only(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertIn('--pub-key', calls[1]); self.assertIn('--signature', calls[1])
    ...
    def test_refuses_world_accessible_or_symlink_helper(self):
        with tempfile.TemporaryDirectory() as folder:
            helper = Path(folder) / 'helper'; helper.write_bytes(b'not executable'); helper.chmod(0o755)
            with self.assertRaisesRegex(RuntimeError, 'owner-only'):
                BACKEND.check_helper(helper)
            helper.chmod(0o700)
            link = Path(folder) / 'link'; link.symlink_to(helper)
            with self.assertRaisesRegex(RuntimeError, 'owner-only'):
