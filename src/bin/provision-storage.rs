...
fn initialize_encrypted_storage(led: &mut Output<'_>) {
    use rissokey::platform::flash_encryption::{self, FlashEncryptionError};
...
fn verify_encrypted_roundtrip(led: &mut Output<'_>) {
    use rissokey::platform::flash_encryption as flash;
...
fn main() -> ! {
    ...
    milestone(9, 0);
    rissokey::platform::storage::write_storage_version(&filesystem).unwrap_or_else(|_| {
        milestone(112, 0);
        signal_result(&mut led, 6);
    });
    milestone(10, 0);
    rissokey::platform::storage::write_persistent_state_initialization_marker(&filesystem)
        .unwrap_or_else(|_| {
            milestone(113, 0);
