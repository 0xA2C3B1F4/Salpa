...
use salpa::platform::{attestation_import, flash_encryption};
...
const EXPECTED_CERTIFICATE: &[u8; 32] = include_bytes!(env!("SALPA_ATTESTATION_CERT_SHA256_FILE"));
const EXPECTED_PUBLIC_KEY: &[u8; 65] = include_bytes!(env!("SALPA_ATTESTATION_PUBLIC_FILE"));
const _: () =
...
struct ImportPlatform<'a> {
    rng: &'a mut salpa::platform::rng::HardwareRng,
    store: salpa::platform::storage::FilesystemSet<'a>,
    ui: ProvisioningUserInterface,
}
impl<'a> trussed::Platform for ImportPlatform<'a> {
    type R = &'a mut salpa::platform::rng::HardwareRng;
    type S = salpa::platform::storage::FilesystemSet<'a>;
...
fn service<'a>(
    rng: &'a mut salpa::platform::rng::HardwareRng,
    store: salpa::platform::storage::FilesystemSet<'a>,
) -> trussed::Service<ImportPlatform<'a>, salpa::platform::dispatch::FidoDispatch> {
    let mut seed = [0u8; 32];
...
fn main() -> ! {
    ...
    let result = (|| -> Result<(), u32> {
        ...
        let mut storage =
            salpa::platform::storage::FidoFlashStorage::new(peripherals.FLASH).map_err(|_| 3u32)?;
        let mut internal_allocation = Allocation::new();
        let internal_filesystem =
            salpa::platform::storage::mount_existing(&mut internal_allocation, &mut storage)
                .map_err(|_| 5u32)?;
        salpa::platform::storage_init::verify_empty_identity_import_target(&internal_filesystem)
            .map_err(|_| 6u32)?;
        ...
        let proof_store = salpa::platform::storage::FilesystemSet::new(
            &volatile_filesystem,
            &volatile_filesystem,
        );

        let source = esp_hal::rng::TrngSource::new(peripherals.RNG, peripherals.ADC1);
        let mut hardware_rng = salpa::platform::rng::HardwareRng::new(source)
        ...
        let inline = salpa::platform::runtime::InlineSyscall::new(proof_service, endpoint);
        let mut client =
        ...
        let persistent_store = salpa::platform::storage::FilesystemSet::new(
            &internal_filesystem,
            &volatile_filesystem,
        );
        ...
        let inline = salpa::platform::runtime::InlineSyscall::new(persistent_service, endpoint);
        let mut client =
            trussed::ClientImplementation::<_, salpa::platform::dispatch::FidoDispatch>::new(
                requester, inline, None,
            );
        ...
        let key_target = PathBuf::from(salpa::platform::attestation::ATTESTATION_KEY_PATH);
        let cert_target = PathBuf::from(salpa::platform::attestation::ATTESTATION_CERT_PATH);
