...
const ATTESTATION_KEY: &[u8; 32] = include_bytes!(env!("SALPA_DEV_ATTESTATION_KEY"));
const ATTESTATION_CERT: &[u8] = include_bytes!(env!("SALPA_DEV_ATTESTATION_CERT"));
...
fn main() -> ! {
    assert!(
        salpa::identity::is_valid_p256_scalar(ATTESTATION_KEY),
        "attestation key is not a valid P-256 scalar"
    );
    assert_eq!(
        salpa::identity::certificate_aaguid(ATTESTATION_CERT),
        Some(salpa::identity::DEVELOPMENT_AAGUID),
    ...
    salpa::platform::storage::write_storage_version(&internal_filesystem)
    ...
    let platform =
        salpa::platform::runtime::EspPlatform::new(hardware_rng, store, ProvisioningUserInterface);
    ...
    let inline = salpa::platform::runtime::InlineSyscall::new(service, endpoint);
    let mut client =
    ...
    salpa::platform::storage::write_persistent_state_initialization_marker(&internal_filesystem)
