...
impl Default for Esp32S2UpdateBackend {
    fn default() -> Self {
        Self::new()
    }
}
...
impl UpdateBackend for Esp32S2UpdateBackend {
    ...
    fn layout(&mut self) -> Result<UpdateLayout, Self::Error> {
        ota::update_layout().map_err(|_| Error::Ota)
    }
    ...
    fn activate(
        ...
        ota::activate_update(slot, sequence, activation_entry).map_err(|_| Error::Ota)
    }
}

pub const fn trusted_update_key_digest() -> [u8; 32] {
    const HEX: &str = env!("RISSO_KEY_UPDATE_KEY_DIGEST_HEX");
