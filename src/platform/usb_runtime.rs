...
impl<'usb, B: UsbBus> UsbRuntime<'usb, B> {
    ...
    pub fn poll(&self) -> Result<Event, UsbRuntimeError> {
    ...
    fn poll_at(&self, now_ms: u32) -> Result<Event, UsbRuntimeError> {
        let mut event = Event::None;
        if self.usb.borrow_mut().poll() {
            let mut report = [0_u8; REPORT_SIZE];
            match self.usb.borrow().receive(&mut report) {
                Ok(true) => {
                    let (received_event, interrupted) = self
                        .ctaphid
                        .borrow_mut()
                        .ingest_with_interruption(&report, now_ms);
                ...
                Ok(false) => {}
                Err(error) => return Err(UsbRuntimeError::Report(error)),
            }
        }
    ...
    fn send_pending(&self) -> Result<(), UsbRuntimeError> {
...
impl<B: UsbBus> UserPresenceWaitHook for UsbRuntime<'_, B> {
    ...
    fn poll_wait(&self) {
        ...
        self.poll_at(now_ms)
            .expect("USB transport failed during user-presence wait");
    }
