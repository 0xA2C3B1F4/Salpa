#[path = "src/build_environment.rs"]
mod build_environment;
use build_environment::{build_var, build_var_os, configure_build_environment};
...
fn main() {
    ...
    require_epoch_preview_acknowledgement();
    require_epoch_maintenance_scope();
    ...
    if std::env::var("CARGO_CFG_TARGET_ARCH").as_deref() == Ok("xtensa") {
        ...
        let protected_s2 = std::env::var_os("CARGO_FEATURE_RELEASE_FLASH_ENCRYPTION").is_some()
            && std::env::var_os("CARGO_FEATURE_FIDO_STACK").is_some()
            && std::env::var_os("CARGO_FEATURE_MCU_ESP32S2").is_some();
        ...
        // The HAL entry script (or its protected S2 wrapper) must be last.
        if protected_s2 {
            println!("cargo:rerun-if-changed=ld/esp32s2-memory-protection.x");
            println!("cargo:rerun-if-changed=ld/esp32s2-protected.x");
            println!("cargo:rustc-link-arg=-Tesp32s2-protected.x");
        } else {
...
fn require_epoch_maintenance_scope() {
    use std::path::PathBuf;

    for name in ["SALPA_EPOCH_MAINTENANCE_ACK", "SALPA_EPOCH_PLAN_PATH"] {
        println!("cargo:rerun-if-env-changed={name}");
    }
    if std::env::var_os("CARGO_FEATURE_SECURITY_EPOCH_MAINTENANCE").is_none() {
        return;
    }
    assert_eq!(
        build_var("SALPA_EPOCH_MAINTENANCE_ACK").as_deref(),
        Ok("COUNTER_ONLY_MAINTENANCE"),
        "maintenance requires explicit COUNTER_ONLY_MAINTENANCE acknowledgement"
    );
    assert!(std::env::var_os("CARGO_FEATURE_MCU_ESP32S2").is_some());
    for feature in [
        "MCU_ESP32S3",
        "SECURITY_EPOCH_PREVIEW",
        "SIGNED_AB_FAILURE_TEST",
        "SIGNED_AB_SUCCESS_TEST",
        ...
        assert!(
            ...
            "counter maintenance must remain a separate device-scoped build"
        );
    }
    let path = PathBuf::from(
        build_var_os("SALPA_EPOCH_PLAN_PATH").expect("maintenance requires a private plan file"),
    );
    let canonical = path
        .canonicalize()
        .expect("cannot resolve maintenance plan");
    let root = PathBuf::from(std::env::var_os("CARGO_MANIFEST_DIR").unwrap())
        .canonicalize()
        .unwrap();
    assert!(
        !canonical.starts_with(root),
        "maintenance plan must remain outside Git"
    );
    let metadata = std::fs::symlink_metadata(&path).expect("cannot inspect maintenance plan");
    assert!(
        metadata.is_file(),
        "maintenance plan must be a regular file"
    );
    #[cfg(unix)]
    {
        ...
        assert!(
            ...
            "maintenance plan must be owner-only"
        );
    }
    #[cfg(not(unix))]
    panic!("maintenance builds require a host with reviewed plan permissions");
    let bytes = std::fs::read(&canonical).expect("cannot read maintenance plan");
    assert!(
        bytes.len() == 164 && &bytes[..8] == b"RKMPLAN1" && bytes[163] == 0,
        "invalid maintenance plan format"
    );
    assert!(
        (1..=16).contains(&bytes[160]),
        "invalid maintenance target epoch"
    );
    let output = PathBuf::from(std::env::var_os("OUT_DIR").unwrap()).join("epoch-plan.bin");
    std::fs::write(output, bytes).expect("cannot embed maintenance plan");
    println!("cargo:rerun-if-changed={}", canonical.display());
}

fn require_epoch_preview_acknowledgement() {
    println!("cargo:rerun-if-env-changed=SALPA_EPOCH_PREVIEW_ACK");
    if std::env::var_os("CARGO_FEATURE_SECURITY_EPOCH_PREVIEW").is_none() {
        return;
    }
    assert_eq!(
        build_var("SALPA_EPOCH_PREVIEW_ACK").as_deref(),
        Ok("READ_EPOCH_ONLY"),
        "epoch preview requires explicit READ_EPOCH_ONLY acknowledgement"
    );
    assert!(
        std::env::var_os("CARGO_FEATURE_MCU_ESP32S2").is_some(),
        "epoch preview requires ESP32-S2"
    );
    for incompatible in [
        "SIGNED_AB_FAILURE_TEST",
        "SIGNED_AB_SUCCESS_TEST",
        ...
        assert!(
            ...
            "epoch preview must remain a separate read-only diagnostic build"
        );
    }
}

fn require_update_key_digest() {
    const NAME: &str = "SALPA_UPDATE_KEY_DIGEST_HEX";
    println!("cargo:rerun-if-env-changed={NAME}");
    if std::env::var_os("CARGO_FEATURE_USB_SIGNED_UPDATE").is_none() {
        return;
    }
    let digest = build_var(NAME).unwrap_or_else(|_| panic!("{NAME} is required"));
    assert!(
        digest.len() == 64
            && digest
                .bytes()
...
fn require_attestation_import_acknowledgement() {
    println!("cargo:rerun-if-env-changed=SALPA_ATTESTATION_IMPORT_ACK");
    if std::env::var_os("CARGO_FEATURE_ATTESTATION_IMPORT").is_none() {
        return;
    }
    assert_eq!(
        build_var("SALPA_ATTESTATION_IMPORT_ACK").as_deref(),
...
fn require_physical_fault_test_acknowledgement() {
    ...
    println!("cargo:rerun-if-env-changed=SALPA_PHYSICAL_FAULT_TEST_ACK");
    if std::env::var_os("CARGO_FEATURE_PHYSICAL_STORAGE_FAULT_TEST").is_none() {
        return;
    }

    assert_eq!(
        build_var("SALPA_PHYSICAL_FAULT_TEST_ACK").as_deref(),
        Ok(ACKNOWLEDGEMENT),
        "physical-storage-fault-test requires SALPA_PHYSICAL_FAULT_TEST_ACK={ACKNOWLEDGEMENT}"
    );
    assert!(
...
fn verify_usb_identity() {
    for variable in ["SALPA_USB_VID", "SALPA_USB_PID", "SALPA_USB_SERIAL"] {
    ...
    let vid = build_var("SALPA_USB_VID").expect("SALPA_USB_VID is required");
    let pid = build_var("SALPA_USB_PID").expect("SALPA_USB_PID is required");
    let serial = build_var("SALPA_USB_SERIAL").expect("SALPA_USB_SERIAL is required");
...
fn require_provisioning_acknowledgement() {
    ...
    println!("cargo:rerun-if-env-changed=SALPA_PROVISIONING_ACK");
    if std::env::var_os("CARGO_FEATURE_STORAGE_PROVISIONING").is_none() {
        return;
    }

    assert_eq!(
        build_var("SALPA_PROVISIONING_ACK").as_deref(),
        Ok(ACKNOWLEDGEMENT),
        "storage-provisioning requires SALPA_PROVISIONING_ACK={ACKNOWLEDGEMENT}"
    );
}
...
fn verify_partition_contract() {
    ...
    println!("cargo:rerun-if-env-changed=SALPA_PARTITION_PROFILE");
    ...
    let profile =
        build_var("SALPA_PARTITION_PROFILE").unwrap_or_else(|_| "legacy-0x8000".to_owned());
    ...
    let path = match profile.as_str() {
        ...
        _ => panic!(
            "SALPA_PARTITION_PROFILE must be legacy-0x8000, e000-migration, signed-ab, or signed-ab-encrypted"
        ),
    };
    assert_eq!(
        encrypted_feature,
