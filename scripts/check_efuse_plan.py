...
def validate_policy(plan: dict[str, object]) -> None:
    ...
    irreversible = {stage_id for stage_id, stage in by_id.items() if stage.get("irreversible") is True}
    require(
    ...
    require(pristine["SPI_BOOT_CRYPT_CNT"] == {"raw_value": "0x0", "writeable": True}, "pristine flash-encryption counter changed")
    require(provisioned["SPI_BOOT_CRYPT_CNT"] == {"raw_value": "0x7", "writeable": True}, "release flash-encryption counter changed")
    require(pristine["SECURE_BOOT_EN"]["value"] is False, "pristine Secure Boot must be disabled")
    require(provisioned["SECURE_BOOT_EN"]["value"] is True, "provisioned Secure Boot must be enabled")
    require(provisioned["ENABLE_SECURITY_DOWNLOAD"]["value"] is False, "full readback must precede ROM lockdown")
    require(provisioned["SECURE_BOOT_AGGRESSIVE_REVOKE"]["value"] is False, "prototype must not enable aggressive key revocation")
