...
Use four-space indentation in Python and Swift. Keep one statement per line,
separate functions with blank lines, and expand nested control flow into readable
blocks. Aim for lines of at most 100 characters. Wrap argument lists and data
structures; retain longer literals when splitting them would obscure their exact
value. Use `cargo fmt` for Rust and preserve the style of vendored code.
...
`tools/check-firmware-builds.py`, including Clippy with `-D warnings`. macOS
helper changes require its fixture suite. Python updates require regenerated
[hash locks](docs/development/python-dependencies.md) and fresh installs.
