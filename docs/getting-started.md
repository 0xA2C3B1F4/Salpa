...
PIP_CACHE_DIR="$TMPDIR/rissokey-pip-cache" python3 -m pip install -r requirements-dev.txt
rustup toolchain install 1.97.0 --profile minimal --component rustfmt
export PYTHONDONTWRITEBYTECODE=1
export CARGO_TARGET_DIR="$TMPDIR/rissokey-host-target"
```
...
```sh
CARGO_TARGET_DIR="$TMPDIR/rissokey-espup-build" cargo +1.97.0 install espup --locked --version 0.17.1
espup install --targets esp32s2,esp32s3 --toolchain-version 1.97.0.0 \
  --name esp-1.97.0.0 --export-file "$TMPDIR/rissokey-export-esp.sh"
. "$TMPDIR/rissokey-export-esp.sh"
python3 tools/check-firmware-builds.py --build-dir "$TMPDIR/rissokey-compile-check"
```

Choose a new build directory. The command links S2 development, protected S2,
S3 bring-up, and S2 attestation-import configurations. It supplies public fixture
inputs, including a synthetic USB identity and signing digest. Outputs are
