...
`SALPA_PHYSICAL_FAULT_TEST_ACK=INTERRUPT_FIDO_STORE`. It also requires the
...
SALPA_MCU=esp32s2 \
SALPA_PROVISIONING_ACK=ERASE_FIDO_STORE \
SALPA_DEV_ATTESTATION_KEY="$TMPDIR/rissokey-development-attestation/attestation-key.raw" \
SALPA_DEV_ATTESTATION_CERT="$TMPDIR/rissokey-development-attestation/attestation-cert.der" \
...
probe set-pin "$SALPA_ASSIGNED_USB_VID" "$SALPA_ASSIGNED_USB_PID" "$state"
probe test-retries "$SALPA_ASSIGNED_USB_VID" "$SALPA_ASSIGNED_USB_PID" "$state"
probe create-resident "$SALPA_ASSIGNED_USB_VID" "$SALPA_ASSIGNED_USB_PID" "$state"
probe assert-discoverable "$SALPA_ASSIGNED_USB_VID" "$SALPA_ASSIGNED_USB_PID" "$state"
probe manage-delete "$SALPA_ASSIGNED_USB_VID" "$SALPA_ASSIGNED_USB_PID" "$state"
probe reset "$SALPA_ASSIGNED_USB_VID" "$SALPA_ASSIGNED_USB_PID" "$state"
probe attest-after-reset "$SALPA_ASSIGNED_USB_VID" "$SALPA_ASSIGNED_USB_PID" "$state"
```
...
## Historical browser interoperability

These observations describe the development-device tests on 2026-09-03, before
the protected-device tests summarized in the
[S2 test results](../testing/esp32s2-results.md). They do not independently
establish browser behavior for the protected configuration or later service
versions.

Chromium on macOS enrolled the physical
prototype through ChatGPT's **Security keys & passkeys** settings. Enrollment
required a security-key PIN. A separate browser session then authenticated
with the enrolled key and reached the signed-in account. This tested later
authentication as well as enrollment. Advanced Account Security was outside
this test's scope.

Safari's initial enrollment request failed with HTTP 400 because a required
page token was absent, before USB or CTAP was invoked. After enrollment in
Chromium, a fresh Safari session authenticated with the same prototype and
reached the signed-in account. The tested Safari enrollment failure occurred
before the authenticator path; successful Safari authentication was a separate
observation. No account identifier, PIN, credential ID or raw WebAuthn packet
is included in this report.

## Remaining electrical test
