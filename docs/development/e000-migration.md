...
contract only with `RISSO_KEY_PARTITION_PROFILE=e000-migration`; the normal
...
python3 tools/build-e000-migration-bootloader.py \
  --idf-path "$IDF_PATH" \
  --build-dir "$RISSO_KEY_BOOTLOADER_BUILD"
```
...
```sh
RISSO_KEY_USB_VID="$RISSO_KEY_TEST_USB_VID" \
RISSO_KEY_USB_PID="$RISSO_KEY_TEST_USB_PID" \
RISSO_KEY_USB_SERIAL="$RISSO_KEY_TEST_USB_SERIAL" \
python3 tools/build-e000-migration-app.py \
  --build-dir "$RISSO_KEY_APPLICATION_BUILD"
```
...
python3 tools/prepare-e000-migration.py \
  --espflash "$ESPFLASH" \
  --bootloader-manifest \
    "$RISSO_KEY_BOOTLOADER_BUILD/rissokey-e000-bootloader-build.json" \
  --application-manifest \
    "$RISSO_KEY_APPLICATION_BUILD/rissokey-e000-application-build.json" \
  --output-dir "$RISSO_KEY_MIGRATION_PACKAGE"
```
...
python3 tools/e000-migration-device.py backup \
  --package-dir "$RISSO_KEY_MIGRATION_PACKAGE" \
  --backup-dir "$RISSO_KEY_PRIVATE_BACKUP" \
...
python3 tools/e000-migration-device.py apply-disposable \
  --package-dir "$RISSO_KEY_MIGRATION_PACKAGE" \
  --evidence-dir "$RISSO_KEY_APPLY_EVIDENCE" \
  --esptool-python "$ESPRESSIF_PYTHON" \
  --port "$RISSO_KEY_ROM_PORT" \
...
python3 tools/e000-migration-device.py restore-disposable \
  --package-dir "$RISSO_KEY_MIGRATION_PACKAGE" \
  --evidence-dir "$RISSO_KEY_RESTORE_EVIDENCE" \
  --esptool-python "$ESPRESSIF_PYTHON" \
  --port "$RISSO_KEY_ROM_PORT" \
