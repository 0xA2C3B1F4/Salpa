...
python3 tools/build-signed-ab-app.py \
  ...
  --secure-version 0 \
  --non-strapping-user-presence \
  --build-dir "$SALPA_AB_BUILD_ROOT/base"
```
...
eFuse security fields were identical. See the
[S2 test summary](../testing/esp32s2-results.md) for the tested configuration.

Trusted firmware signing now uses the local macOS Keychain helper and requires fresh user authentication. See [key management](../guides/key-management.md). The untrusted key remains an isolated negative-test fixture.
