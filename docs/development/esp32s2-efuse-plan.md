...
a blocked final hardening stage. Do not reprovision an enrolled account key
through this template. The plan is deliberately marked `draft-unapproved`.
Committing or validating it is not authorization to
...
- Full ROM download remains available in this pre-lockdown checkpoint. The final
...
Keep the device flash key outside Git while the recovery model requires it.
Retention or deletion is a separate decision after the final recovery model and
required backups are verified. The signing private key also stays outside Git.
Public source and test reports may contain public-key digests, but not private
...
## Tested prototype and package preparation

The [S2 test results](../testing/esp32s2-results.md) summarize package
verification, Secure Boot V2 and release-mode flash-encryption readback,
protected boot, credential persistence, signed USB updates, rollback and
encrypted ROM recovery. Full ROM download remained available in the tested
configuration. The subsequent [final hardening results](../testing/esp32s2-final-hardening.md)
record the completed USB ROM-closure test. This provisioning template remains
a pre-lockdown plan and does not authorize that separate stage.

The `provisioned_before_rom_lockdown` profile describes security fields; it does
not by itself prove credential readiness. The checked-in plan remains a
`draft-unapproved` template, not a record of device-specific authorization.
...
acknowledgement `SALPA_PROVISIONING_ACK=ERASE_FIDO_STORE`. This permits
...
The plan, validator, and Salpa integration code are MIT-licensed. Espressif
