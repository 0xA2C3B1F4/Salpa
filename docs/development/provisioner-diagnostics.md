...
The on-device pattern probe then returned ten pulses, isolating a decrypted
readback mismatch despite successful ROM writes. A second mapping defect was
identified in the pinned IDF `components/soc/esp32s2/include/soc/ext_mem_defs.h`
...
it does not add this flag. The shared helper now passes `1 << 15` instead of
zero for every fixed and temporary mapping. The owner then observed two pulses
with this correction on 2026-09-05. Full raw storage readback changed from its
erased state, and eFuse readback remained unchanged. This passes the storage
initialization gate, while normal FIDO acceptance remains blocked by the
missing original attestation identity.
