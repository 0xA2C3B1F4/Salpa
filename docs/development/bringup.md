...
The initial ESP32-S3 bring-up used a Viewe 4.3-inch development display.
The working FIDO prototype is now ESP32-S2; see the
[S2 test summary](../testing/esp32s2-results.md). Display, touch UI, Wi-Fi and
BLE remain outside the project scope. The following record describes the
earlier S3 board investigation.

```text
Board: Viewe UEDX48270043E-WB-A
ESP32-S3: revision v0.2 observed by espflash
Flash size: 16 MB observed by espflash
PSRAM: 8 MB stated by the vendor; not used or probed by Salpa
...
```sh
SALPA_BUILD_ROOT="$TMPDIR/rissokey-build"
mkdir -p "$SALPA_BUILD_ROOT"
CARGO_TARGET_DIR="$SALPA_BUILD_ROOT/target" ./tools/cargo-esp check --locked
```
