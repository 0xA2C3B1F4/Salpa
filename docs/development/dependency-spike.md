...
patch. RissoKey now vendors that exact dual-licensed revision and applies one
...
Most FIDO code is not reachable from the current entry point and the linker can remove it. These figures prove successful target linkage, not final firmware size or RAM use.
...
production USB VID/PID is assigned. Native USB enumeration has not yet been
attempted through the board's separate `USB` connector.
