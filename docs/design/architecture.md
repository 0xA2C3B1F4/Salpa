...
coverage target. [Evidence](../evidence/README.md) distinguishes the two.
...
- FIDO core: pinned SoloKeys/Trussed revisions, starting with the
  known-compatible set recorded in the dependency spike. RissoKey vendors the pinned
...
- The pinned authenticator exposes its CTAPHID `App` implementation only through
  ...
  but RissoKey exposes no CCID, APDU, or CTAP1 transport and does not validate it.
