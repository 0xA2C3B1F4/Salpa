...
Protected S2 tests verified physically approved identity import, expected
attestation and credential signatures. The same credential later survived
signed updates, rollback and encrypted ROM recovery. See the
[S2 test summary](../testing/esp32s2-results.md). These prototype results do not
replace a restore test with your own recovery material.
...
If the original key cannot be recovered, obtain an explicit decision from the device operator
...
`SALPA_ATTESTATION_IMPORT_ACK=INSTALL_ATTESTATION` and the protected encrypted
...
`SALPA_ATTESTATION_CERT_SHA256_FILE` and `SALPA_ATTESTATION_PUBLIC_FILE`.
