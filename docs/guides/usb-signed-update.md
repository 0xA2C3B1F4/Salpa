...
Use `python-fido2` 2.2.1 and the assigned device VID/PID:
...
Encrypted inactive-slot writing and decrypted readback remain unproven on
protected hardware. That test requires separate authorization for the
irreversible security configuration. This development-device result neither
changes nor authorizes eFuses.
