# Installing and updating RissoKey

RissoKey currently provides separate command-line tools. There is no unified
...
| Update an already working RissoKey | Obtain the publisher's signed application and use the USB update procedure below; no private firmware keys or macOS key helper needed |
...
## Working device: signed USB update
...
3. With the host dependencies installed, inspect the image without USB access:
   ...
   ```sh
   python3 tools/rissokey-usb-ota.py \
     "$RISSO_KEY_ASSIGNED_USB_VID" "$RISSO_KEY_ASSIGNED_USB_PID" \
     "$RISSO_KEY_SIGNED_UPDATE_IMAGE" --dry-run
   ```
