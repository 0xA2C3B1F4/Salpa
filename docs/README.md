...
- [ESP32-S2 memory protection](design/esp32s2-memory-protection.md): PMS RAM permissions, stack guard and pending device acceptance.
- [Physical-attack applicability](design/physical-attack-applicability.md): S2 flash access, AR2022-003, AR2023-007 and the limits of other-model results.
- [Recovery authorization](design/recovery-authorization.md): macOS companion decision, device-enforced permission and FIDO-state replay limits.
...
- [ESP32-S2 hardening design](plans/esp32s2-hardening.md): ROM policy, security epochs, read-only USB status and recovery dependencies.
- [PIN/UV token security](development/pin-token-security.md): lifecycle fixes, PIN-state replay test, counter-design requirements and the ROM-closure decision gate.
- [Release hardening](plans/release-hardening.md): remaining production requirements.

## Testing and development

The [testing guide](testing/README.md) covers reproducible host and build
checks. The [ESP32-S2 test results](testing/esp32s2-results.md) describe hardware
validation, manual browser test results and remaining limits.
The [final S2 hardening report](testing/esp32s2-final-hardening.md) covers the
tested USB ROM closure, post-closure updates, fallback and PIN authentication.
[Change history](changelog.md) records source milestones.
